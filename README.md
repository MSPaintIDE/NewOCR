...
After the image went through any necessary preprocessing phases and is black and white, it needs to detect the bounds of the lines and characters. It does this by finding vertical separations to find the general character lines, then grouping touching black pixels together. Different pieces of multipart letters are identified and put together using thresholds created automatically during training based on the font. A limitation of the current system is the lack of having a 'custom' character not in the training constant string.
